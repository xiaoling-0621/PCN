...
class Exp_Main(Exp_Basic):
    ...
    def test(self, setting, test=0):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if test:
                    if i % 20 == 0:
                        ...
                        input_ = input.mean(0).mean(-1)
                        true_ = true.mean(0).mean(-1)
                        pred_ = pred.mean(0).mean(-1)
                        gt = np.concatenate((input_, true_), axis=0)
                        pd = np.concatenate((input_, pred_), axis=0)
                        visual(gt, pd,self.args.model, os.path.join(folder_path, str(i) + f'.pdf'))
        ...
        f = open(f"{self.args.features}/{self.args.model}/{self.args.model}_{self.args.model_id}.txt", 'a')
        f.write(setting + "  \n")
